...
activation_stack = []  # This is the handle to the namespaces in the heap that holds the
...
def activeNS():
    ...
    if len(activation_stack) == 0 :
      ...
      return activation_stack[-1] # returns the right-most namespace of activation stack
...
def initializeHeap():
    ...
    heap = {}
    # ns = allocateNS()
    activation_stack.append(allocateNS())  # create namespace in  heap  for global variables
    declare("h0", "parentns", "nil")  # initialize the activation stack with handle h0

def printHeap(): 
    """prints contents of activation_stack  and  heap"""
...
def allocateNS() :
    ...
    # declare(newloc, "parentns", activeNS())
    return newloc
...
def declare(handle, field, rval) :
    ...
    ## WRITE ME:
    # pass
    if isLValid(handle, field) : # check if the variable to declare does exist
        crash("redeclaration error: " + handle + " " + field)
    else : 
...
def update(handle, field, rval) :
    """updates  rval  at heap[handle][field], provided that
       ...
       params:  handle, field, as described above
                rval -- an int or a handle
    """
    ## enforece the variables to be declared first
    if isLValid(handle, field) : 
        # heap[handle][field] = rval
        ...
        crash("declaration needed");

def push(handle) :
    """push the handle into activation stack"""
    activation_stack.append(handle)

def pop() :
    """pop the active handle out of the activation stack"""
